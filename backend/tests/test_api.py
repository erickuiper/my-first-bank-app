...
async def test_register_user_duplicate_email():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        unique_email = get_unique_email()
        response1 = await ac.post("/api/v1/auth/register", json={
            "email": unique_email,
            "password": "testpassword123"
        })
...
async def test_login_user():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        unique_email = get_unique_email()
        register_response = await ac.post("/api/v1/auth/register", json={
            "email": unique_email,
            "password": "testpassword123"
        })
...
async def test_protected_endpoint_with_invalid_token():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        response = await ac.get("/api/v1/children/", headers={
...
async def test_create_child_with_auth():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.post("/api/v1/children/", 
            json={
                "name": "Test Child",
...
async def test_transaction_pagination():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        child_response = await ac.post("/api/v1/children/", 
            json={
                "name": "Test Child",
                "birthdate": "2015-01-01"
            },
        ...
        response = await ac.get(f"/api/v1/accounts/{account_id}/transactions?limit=10", 
            headers={"Authorization": f"Bearer {token}"}
        )
...
async def test_transaction_pagination_with_cursor():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.get(f"/api/v1/accounts/{account_id}/transactions?limit=5&cursor=invalid_cursor", 
            headers={"Authorization": f"Bearer {token}"}
        )
...
async def test_deposit_with_idempotency():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.post(f"/api/v1/accounts/{account_id}/deposit", 
            json={
                "amount_cents": 1000,
...
async def test_deposit_duplicate_idempotency():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        idempotency_key = f"test_key_{uuid.uuid4().hex[:8]}"
        response1 = await ac.post(f"/api/v1/accounts/{account_id}/deposit", 
            json={
                "amount_cents": 1000,
                "idempotency_key": idempotency_key
            },
        ...
        response2 = await ac.post(f"/api/v1/accounts/{account_id}/deposit", 
            json={
                "amount_cents": 1000,
                "idempotency_key": idempotency_key
            },
...
async def test_deposit_invalid_account():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.post("/api/v1/accounts/99999/deposit", 
            json={
                "amount_cents": 1000,
                "idempotency_key": f"test_key_{uuid.uuid4().hex[:8]}"
            },
...
async def test_deposit_unauthorized_account():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        register_response2 = await ac.post("/api/v1/auth/register", json={
            "email": unique_email2,
            "password": "testpassword123"
        })
        assert register_response2.status_code == 200
        
        login_response2 = await ac.post("/api/v1/auth/login", json={
            "email": unique_email2,
            "password": "testpassword123"
        })
...
async def test_transactions_invalid_account():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.get("/api/v1/accounts/99999/transactions", 
            headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 404
        assert "Account not found" in response.json()["detail"]
...
async def test_transactions_unauthorized_account():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.get(f"/api/v1/accounts/{account_id}/transactions", 
            headers={"Authorization": f"Bearer {token2}"}
        )
        assert response.status_code == 404
        assert "Account not found" in response.json()["detail"]
...
async def test_transactions_with_valid_cursor():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as ac:
        ...
        response = await ac.get(f"/api/v1/accounts/{account_id}/transactions?limit=1", 
            headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data["transactions"]) == 1
        assert data["has_more"] == False  # Should be false since we only have 1 transaction
        
        # Test with cursor if available
        if data["next_cursor"]:
            response2 = await ac.get(f"/api/v1/accounts/{account_id}/transactions?limit=1&cursor={data['next_cursor']}", 
                headers={"Authorization": f"Bearer {token}"}
            )
            assert response2.status_code == 200
